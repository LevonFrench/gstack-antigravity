...
```bash
_UPD=$(~/.gemini/antigravity/skills/gstack/bin/gstack-update-check 2>/dev/null || .gemini/antigravity/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.gstack/sessions
touch ~/.gstack/sessions/"$PPID"
_SESSIONS=$(find ~/.gstack/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.gstack/sessions -mmin +120 -type f -delete 2>/dev/null || true
_CONTRIB=$(~/.gemini/antigravity/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || true)
_PROACTIVE=$(~/.gemini/antigravity/skills/gstack/bin/gstack-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
source <(~/.gemini/antigravity/skills/gstack/bin/gstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(~/.gemini/antigravity/skills/gstack/bin/gstack-config get telemetry 2>/dev/null || true)
...
for _PF in ~/.gstack/analytics/.pending-*; do [ -f "$_PF" ] && ~/.gemini/antigravity/skills/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```

If `PROACTIVE` is `"false"`, do not proactively suggest gstack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.gemini/antigravity/skills/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue.
...
If A: run `~/.gemini/antigravity/skills/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `~/.gemini/antigravity/skills/gstack/bin/gstack-config set telemetry anonymous`
If B→B: run `~/.gemini/antigravity/skills/gstack/bin/gstack-config set telemetry off`
...
Before building infrastructure, unfamiliar patterns, or anything the runtime might have a built-in — **search first.** Read `~/.gemini/antigravity/skills/gstack/ETHOS.md` for the full philosophy.
...
~/.gemini/antigravity/skills/gstack/bin/gstack-telemetry-log \
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.gemini/antigravity/skills/gstack/browse/dist/browse" ] && B="$_ROOT/.gemini/antigravity/skills/gstack/browse/dist/browse"
[ -z "$B" ] && B=~/.gemini/antigravity/skills/gstack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
